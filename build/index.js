...
var _require = __webpack_require__(11),
    cordsFromAddress = _require.cordsFromAddress;
...
module.exports = function (_React$Component) {
  ...
  _createClass(_class, [{
    ...
    value: function componentDidMount() {
      ...
      if (this.props.onIdle) window.google.maps.event.addListener(this.map, 'idle', function () {
        return _this2.props.onIdle(_this2.map, _this2.allMarkers);
      });
      // Add markers to map
      this.setMarkers();
    }
    ...
    value: function componentDidUpdate(prevProps, prevState) {
      ...
      if (JSON.stringify(this.state) !== JSON.stringify(prevState)) {
        //handle zoom changes
        if (this.map.getZoom() !== this.state.zoom) this.map.setZoom(this.state.zoom);
        //handle center changes
        if (JSON.stringify(prevState.center) !== JSON.stringify(this.state.center)) this.map.panTo(this.state.center);
        //handle changes to markers (can only add markers)
        if (JSON.stringify(this.state.markers) !== JSON.stringify(prevState.markers)) this.setMarkers();
      }
    }
  }, {
    key: 'setCenter',
    value: function setCenter() {
      ...
      if (this.props.center) {
        ...
        if (typeof this.props.center === 'string') {
          ...
          cordsFromAddress(this.props.center, this.props.apiKey).then(function (cords) {
            console.log(cords);
            _this3.setState({ center: cords });
          });
        } else {
          // else set state
    ...
    value: function setMarkers() {
      ...
      this.state.markers.forEach(function (marker, i) {
        ...
        if (typeof marker.position === 'string') {
          ...
          cordsFromAddress(marker.position, _this4.props.apiKey).then(function (cords) {
            marker.position = cords;
            //set the marker
            _this4.setMarker(marker);
          });
          // ... if it is an object then the position is cordinates
        } else if (_typeof(marker.position) === 'object') {
          // set the marker
          _this4.setMarker(marker);
        }
      });
    ...
    value: function setMarker(marker) {
      ...
      var m = new window.google.maps.Marker({
        ...
        animation: marker.animation === 'drop' ? window.google.maps.Animation.DROP : marker.animation === 'bounce' ? window.google.maps.Animation.BOUNCE : null,
        title: marker.title,
        label: marker.label,
        map: this.map,
        icon: marker.icon
...
exports.cordsFromAddress = function (addr, GOOGLE_API_KEY) {
  return axios.get('https://maps.googleapis.com/maps/api/geocode/json?address=' + addr + '&key=' + GOOGLE_API_KEY).then(function (res) {
